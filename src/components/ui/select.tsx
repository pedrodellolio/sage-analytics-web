...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        "border-input text-foreground data-[placeholder]:text-foreground/60 focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive flex h-9 w-full items-center justify-between gap-2 rounded-md border bg-transparent px-3 py-2 text-sm shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&>span]:line-clamp-1",
        className
      )}
      {...props}
    >
      ...
      <SelectPrimitive.Icon asChild>
        <ChevronDownIcon
          size={16}
          className="text-foreground/60/80 in-aria-invalid:text-destructive/80 shrink-0"
        />
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.Label
      ...
      className={cn(
        "text-foreground/60 py-1.5 ps-8 pe-2 text-xs font-medium",
        className
      )}
      {...props}
    />
    ...
    >
      <span className="absolute start-2 flex size-3.5 items-center justify-center">
        <SelectPrimitive.ItemIndicator>
          <CheckIcon size={16} color="var(--foreground)" />
        </SelectPrimitive.ItemIndicator>
      </span>
...
function SelectScrollUpButton({
  ...
  return (
    <SelectPrimitive.ScrollUpButton
      ...
      className={cn(
        "text-foreground/60/80 flex cursor-default items-center justify-center py-1",
        className
      )}
      {...props}
    >
